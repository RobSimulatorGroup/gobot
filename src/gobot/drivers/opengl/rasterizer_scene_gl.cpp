...
void GLRasterizerScene::EnsureDefaultProgram() {
    ...
    GLuint vs = CompileShader(GL_VERTEX_SHADER, DEFAULT_MESH_VERT);
    GLuint fs = CompileShader(GL_FRAGMENT_SHADER, DEFAULT_MESH_FRAG);
    if (vs == 0 || fs == 0) {
        return;
    }
...
void GLRasterizerScene::DrawVisualItem(const VisualMeshRenderItem& item) {
    ...
    UploadMesh(mesh);
    const Color& color = item.surface_color;
    ...
    glUniform4f(glGetUniformLocation(default_program_, "u_color"),
                ...
                color.blue(),
                color.alpha());
    glUniform1f(glGetUniformLocation(default_program_, "u_metallic"), static_cast<float>(item.metallic));
    glUniform1f(glGetUniformLocation(default_program_, "u_roughness"), static_cast<float>(item.roughness));
    glUniform1f(glGetUniformLocation(default_program_, "u_specular"), static_cast<float>(item.specular));
