...
#include "gobot/rendering/debug_draw/pack_rect.hpp"
...
uint32_t genSphere(uint8_t _subdiv0, void *_pos0 = NULL, uint16_t _posStride0 = 0, void *_normals0 = NULL,
    ...
    if (NULL != _pos0) {
        struct Gen {
            ...
            void
            triangle(const bx::Vec3 &_v0, const bx::Vec3 &_v1, const bx::Vec3 &_v2, float _scale, uint8_t _subdiv) {
                if (0 == _subdiv) {
                    addVert(_v0);
                    addVert(_v1);
                    addVert(_v2);
...
struct GeometryT {
    ...
    GeometryHandle create(uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices, const void *_indices,
                          bool _index32) {
        ...
        if (isValid(handle)) {
            ...
            geometry.m_vbh = bgfx::createVertexBuffer(
                    bgfx::copy(_vertices, _numVertices * sizeof(DdVertex)), DebugMeshVertex::ms_layout
            );

            geometry.m_topologyNumIndices[0] = _numIndices;
            geometry.m_topologyNumIndices[1] = bgfx::topologyConvert(
                    bgfx::TopologyConvert::TriListToLineList, NULL, 0, _indices, _numIndices, _index32
            );
            ...
            bgfx::topologyConvert(
                    bgfx::TopologyConvert::TriListToLineList, &indexData[geometry.m_topologyNumIndices[0] * indexSize],
                    geometry.m_topologyNumIndices[1] * indexSize, _indices, _numIndices, _index32
            );

            geometry.m_ibh = bgfx::createIndexBuffer(
                    mem, _index32 ? BGFX_BUFFER_INDEX32 : BGFX_BUFFER_NONE
            );
        }
...
struct DebugDrawShared {
    void init(bx::AllocatorI *_allocator) {
        ...
        m_texture = bgfx::createTexture2D(SPRITE_TEXTURE_SIZE, SPRITE_TEXTURE_SIZE, false, 1,
        ...
        for (uint32_t mesh = 0; mesh < 4; ++mesh) {
            ...
            uint32_t numLineListIndices = bgfx::topologyConvert(
                    bgfx::TopologyConvert::TriListToLineList, NULL, 0, trilist, numIndices, false
            );
            ...
            bgfx::topologyConvert(
                    bgfx::TopologyConvert::TriListToLineList, &indicesOut[numIndices],
...
//				index[num*6+ii*6+5] = uint16_t(ii+num);
        ...
        for (uint32_t mesh = DebugMesh::Sphere0; mesh < DebugMesh::Quad; ++mesh) {
            ...
            bx::memCopy(&ib->data[m_mesh[id].m_startIndex[0] * sizeof(uint16_t)], indices[id],
        ...
        bx::memCopy(&ib->data[m_mesh[DebugMesh::Quad].m_startIndex[0] * sizeof(uint16_t)], s_quadIndices,
        ...
        bx::memCopy(&ib->data[m_mesh[DebugMesh::Cube].m_startIndex[0] * sizeof(uint16_t)], s_cubeIndices,
    ...
    SpriteHandle createSprite(uint16_t _width, uint16_t _height, const void *_data) {
        ...
        if (isValid(handle)) {
            ...
            bgfx::updateTexture2D(
                    m_texture, 0, 0, pack.m_x, pack.m_y, pack.m_width, pack.m_height,
    ...
    GeometryHandle
    createGeometry(uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices, const void *_indices,
                   bool _index32) {
...
struct DebugDrawEncoderImpl {
    ...
    void draw(const bx::Sphere &_sphere) {
        const Attrib &attrib = m_attrib[m_stack];
        float mtx[16];
        bx::mtxSRT(mtx, _sphere.radius, _sphere.radius, _sphere.radius, 0.0f, 0.0f, 0.0f, _sphere.center.x,
                   _sphere.center.y, _sphere.center.z
        );
    ...
    void draw(GeometryHandle _handle) {
        ...
        if (wireframe) {
            m_encoder->setIndexBuffer(
                    geometry.m_ibh, geometry.m_topologyNumIndices[0], geometry.m_topologyNumIndices[1]
            );
        } else if (0 != geometry.m_topologyNumIndices[0]) {
            m_encoder->setIndexBuffer(
                    geometry.m_ibh, 0, geometry.m_topologyNumIndices[0]
            );
        }
    ...
    void draw(bool _lineList, uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices,
            ...
            if (0 < _numIndices) {
                ...
                if (!_lineList && wireframe) {
                    numIndices = bgfx::topologyConvert(
                            bgfx::TopologyConvert::TriListToLineList, NULL, 0, _indices, _numIndices, false
                    );

                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
                    bgfx::topologyConvert(
                            bgfx::TopologyConvert::TriListToLineList, tib.data, numIndices * sizeof(uint16_t), _indices,
                            _numIndices, false
                    );
                } else {
                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
    ...
    void drawFrustum(const float *_viewProj) {
        bx::Plane planes[6] = {bx::init::None, bx::init::None, bx::init::None, bx::init::None, bx::init::None,
                               bx::init::None};
    ...
    void drawArc(Axis _axis, float _x, float _y, float _z, float _radius, float _degrees) {
        ...
        bx::Vec3 pos = getPoint(
                _axis, bx::sin(step * 0) * _radius, bx::cos(step * 0) * _radius
        );
        ...
        for (uint32_t ii = 1; ii < n + 1; ++ii) {
            pos = getPoint(
                    _axis, bx::sin(step * ii) * _radius, bx::cos(step * ii) * _radius
            );
        ...
        pos = getPoint(
                _axis, bx::sin(step * n) * _radius, bx::cos(step * n) * _radius
        );
        moveTo({pos.x + _x, pos.y + _y, pos.z + _z});
        lineTo(_x, _y, _z);
    }
    ...
    void drawCircle(Axis _axis, float _x, float _y, float _z, float _radius, float _weight) {
        ...
        bx::Vec3 pos = getPoint(
                _axis, bx::lerp(xy0[0], xy1[0], _weight) * _radius, bx::lerp(xy0[1], xy1[1], _weight) * _radius
        );
