...
bool WriteLittleEndian(std::ostream& stream, T value) {
    ...
    std::array<char, sizeof(T)> bytes = std::bit_cast<std::array<char, sizeof(T)>>(value);
...
bool CanWriteFloat(RealType value) {
    return std::isfinite(value) &&
           std::abs(value) <= static_cast<RealType>(std::numeric_limits<float>::max());
}

bool CanWriteVector2(const Vector2& value) {
    return CanWriteFloat(value.x()) && CanWriteFloat(value.y());
}

bool CanWriteVector3(const Vector3& value) {
    return CanWriteFloat(value.x()) && CanWriteFloat(value.y()) && CanWriteFloat(value.z());
}

bool IsGobotBinaryPLY(const std::string& path) {
    if (ToLower(GetFileExtension(path)) != "ply") {
        return false;
    }
    ...
    return header.starts_with("ply\nformat binary_little_endian 1.0\n") &&
           header.find("comment Generated by Gobot\n") != std::string::npos;
}
...
Ref<Resource> ResourceFormatLoaderMesh::Load(const std::string& path,
    ...
    const bool is_gobot_sidecar = IsGobotBinaryPLY(global_path);
    // Gobot's binary PLY sidecars are already indexed triangle meshes. Assimp's
    // cache-locality pass assumes JoinIdenticalVertices ran first, while that
    // pass is both unnecessary here and unstable on large verbose PLY meshes.
    unsigned int postprocess_flags = aiProcess_Triangulate;
    if (!is_gobot_sidecar) {
        postprocess_flags |= aiProcess_JoinIdenticalVertices |
                             aiProcess_DropNormals |
                             aiProcess_GenSmoothNormals |
                             aiProcess_CalcTangentSpace |
                             aiProcess_ImproveCacheLocality;
    }
    const aiScene* scene = importer.ReadFile(global_path, postprocess_flags);
    ...
    AddMeshRecursive(scene,
                     ...
                     surfaces);
    if (is_gobot_sidecar) {
        // Assimp attaches a synthesized default material to PLY meshes. Gobot
        // sidecars are geometry-only; retaining that material would override
        // MeshInstance3D and ArrayMesh fallback colors after a round trip.
...
bool ResourceFormatSaverPLY::Save(const Ref<Resource>& resource,
                                  const std::string& path,
                                  ResourceSaverFlags flags) {
    (void)flags;

    const Ref<ArrayMesh> mesh = dynamic_pointer_cast<ArrayMesh>(resource);
    if (!mesh.IsValid()) {
        return false;
    }

    const MeshSurfaceList surfaces = mesh->GetSurfaces();
    if (surfaces.size() != 1) {
        LOG_ERROR("Cannot save ArrayMesh '{}': binary PLY persistence requires exactly one surface.", path);
        return false;
    }

    const MeshSurfaceData& surface = surfaces.front();
    const std::size_t vertex_count = surface.vertices.size();
    if (vertex_count == 0 || surface.indices.empty() || surface.indices.size() % 3 != 0) {
        LOG_ERROR("Cannot save ArrayMesh '{}': binary PLY persistence requires indexed triangle geometry.", path);
        return false;
    }
    if (vertex_count > std::numeric_limits<std::uint32_t>::max() ||
        surface.indices.size() / 3 > std::numeric_limits<std::uint32_t>::max()) {
        LOG_ERROR("Cannot save ArrayMesh '{}': mesh exceeds binary PLY size limits.", path);
        return false;
    }
    if (std::ranges::any_of(surface.indices,
                            [vertex_count](std::uint32_t index) { return index >= vertex_count; }) ||
        !std::ranges::all_of(surface.vertices, CanWriteVector3)) {
        LOG_ERROR("Cannot save ArrayMesh '{}': mesh contains an invalid vertex or index.", path);
        return false;
    }

    const bool has_normals = surface.normals.size() == vertex_count;
    const bool has_uv = surface.uv0.size() == vertex_count;
    const bool has_colors = surface.colors.size() == vertex_count;
    if ((has_normals && !std::ranges::all_of(surface.normals, CanWriteVector3)) ||
        (has_uv && !std::ranges::all_of(surface.uv0, CanWriteVector2)) ||
        (has_colors && std::ranges::any_of(surface.colors, [](const Color& color) {
             return !std::isfinite(color.red()) || !std::isfinite(color.green()) ||
                    !std::isfinite(color.blue()) || !std::isfinite(color.alpha());
         }))) {
        LOG_ERROR("Cannot save ArrayMesh '{}': mesh contains a non-finite vertex attribute.", path);
        return false;
    }

    const std::string global_path = ProjectSettings::HasInstance()
                                            ? ProjectSettings::GetInstance()->GlobalizePath(path)
                                            : path;
    const std::filesystem::path output_path(global_path);
    std::error_code directory_error;
    if (!output_path.parent_path().empty()) {
        std::filesystem::create_directories(output_path.parent_path(), directory_error);
    }
    if (directory_error) {
        LOG_ERROR("Cannot create directory for binary PLY '{}': {}.", path, directory_error.message());
        return false;
    }

    std::ofstream stream(output_path, std::ios::binary | std::ios::trunc);
    if (!stream.is_open()) {
        LOG_ERROR("Cannot open binary PLY '{}' for writing.", path);
        return false;
    }

    stream << "ply\n"
           << "format binary_little_endian 1.0\n"
           << "comment Generated by Gobot\n"
           << "element vertex " << vertex_count << "\n"
           // Keep the first payload byte out of CR/LF range. Assimp's streamed
           // PLY reader otherwise consumes a leading 0x0a/0x0d byte as another
           // header line ending, which corrupts the entire binary payload.
           << "property uchar gobot_padding\n"
           << "property float x\n"
           << "property float y\n"
           << "property float z\n";
    if (has_normals) {
    ...
    if (has_uv) {
        stream << "property float texture_u\n"
    ...
    stream << "element face " << surface.indices.size() / 3 << "\n"
           << "property list uchar uint vertex_indices\n"
           << "end_header\n";

    for (std::size_t i = 0; i < vertex_count && stream.good(); ++i) {
        constexpr std::uint8_t vertex_padding = 0;
        stream.write(reinterpret_cast<const char*>(&vertex_padding), sizeof(vertex_padding));
        const Vector3& vertex = surface.vertices[i];
        WriteLittleEndian(stream, static_cast<float>(vertex.x()));
        WriteLittleEndian(stream, static_cast<float>(vertex.y()));
        WriteLittleEndian(stream, static_cast<float>(vertex.z()));
        if (has_normals) {
            const Vector3& normal = surface.normals[i];
            WriteLittleEndian(stream, static_cast<float>(normal.x()));
            WriteLittleEndian(stream, static_cast<float>(normal.y()));
            WriteLittleEndian(stream, static_cast<float>(normal.z()));
        }
        if (has_uv) {
            const Vector2& uv = surface.uv0[i];
            WriteLittleEndian(stream, static_cast<float>(uv.x()));
            WriteLittleEndian(stream, static_cast<float>(uv.y()));
        }
        if (has_colors) {
            const Color& color = surface.colors[i];
            const std::array<std::uint8_t, 4> channels{
                    ColorChannelToByte(color.red()),
                    ColorChannelToByte(color.green()),
                    ColorChannelToByte(color.blue()),
                    ColorChannelToByte(color.alpha()),
            };
            stream.write(reinterpret_cast<const char*>(channels.data()),
                         static_cast<std::streamsize>(channels.size()));
        }
    }

    for (std::size_t i = 0; i < surface.indices.size() && stream.good(); i += 3) {
        constexpr std::uint8_t triangle_size = 3;
        stream.write(reinterpret_cast<const char*>(&triangle_size), sizeof(triangle_size));
        WriteLittleEndian(stream, surface.indices[i]);
        WriteLittleEndian(stream, surface.indices[i + 1]);
        WriteLittleEndian(stream, surface.indices[i + 2]);
    }

    if (!stream.good()) {
        LOG_ERROR("Failed while writing binary PLY '{}'.", path);
        return false;
    }
    return true;
}

bool ResourceFormatSaverPLY::Recognize(const Ref<Resource>& resource) const {
    return dynamic_pointer_cast<ArrayMesh>(resource).IsValid();
}

void ResourceFormatSaverPLY::GetRecognizedExtensions(const Ref<Resource>& resource,
                                                      std::vector<std::string>* extensions) const {
    if (Recognize(resource)) {
        extensions->push_back("ply");
    }
}
...
GOBOT_REGISTRATION {
    ...
    Class_<gobot::ResourceFormatSaverPLY>("ResourceFormatSaverPLY")
            .constructor()(CtorAsRawPtr);
};
