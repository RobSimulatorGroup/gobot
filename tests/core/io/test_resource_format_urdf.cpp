...
TEST(TestResourceFormatURDF, resolves_package_directory_without_package_manifest) {
    const std::filesystem::path temp_root =
            std::filesystem::temp_directory_path() / "gobot_urdf_manifestless_package_test";
    const std::filesystem::path project_path = temp_root / "project";
    const std::filesystem::path package_path = temp_root / "robot_pkg";
    const std::filesystem::path mesh_path = package_path / "meshes" / "base_visual.stl";
    const std::filesystem::path urdf_path = package_path / "urdf" / "robot.urdf";
