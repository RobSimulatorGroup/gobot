...
the runtime pose. Playback loads the balanced policy from
`res://policies/go1_velocity.onnx`; `GOBOT_GO1_POLICY` can explicitly select a
different manifest-backed `.onnx` or `.pt` during development. Missing, legacy,
or contract-mismatched policies fail at startup.
